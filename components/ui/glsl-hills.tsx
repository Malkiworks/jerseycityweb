        ...
        return new THREE.Mesh(
          ...
          new THREE.RawShaderMaterial({
            ...
            vertexShader: `
              ...
              float cnoise(vec3 P){
                vec3 Pi0=floor(P); vec3 Pi1=Pi0+vec3(1.); Pi0=mod289(Pi0); Pi1=mod289(Pi1);
                ...
                vec4 gx0=ixy0*(1./7.); vec4 gy0=fract(floor(gx0)*(1./7.))-.5; gx0=fract(gx0); vec4 gz0=vec4(.5)-abs(gx0)-abs(gy0); vec4 sz0=step(gz0,vec4(0.)); gx0-=sz0*(step(0.,gx0)-.5); gy0-=sz0*(step(0.,gy0)-.5);
                vec4 gx1=ixy1*(1./7.); vec4 gy1=fract(floor(gx1)*(1./7.))-.5; gx1=fract(gx1); vec4 gz1=vec4(.5)-abs(gx1)-abs(gy1); vec4 sz1=step(gz1,vec4(0.)); gx1-=sz1*(step(0.,gx1)-.5); gy1-=sz1*(step(0.,gy1)-.5);
              ...
              void main(void){
                vec3 p=(rotateMatrixX(radians(90.))*vec4(position,1.)).xyz;
                float sin1=sin(radians(p.x/128.*90.));
                vec3 nPos=p+vec3(0.,0.,time*-30.);
                float n1=cnoise(nPos*.08), n2=cnoise(nPos*.06), n3=cnoise(nPos*.4);
                vec3 last=p+vec3(0., n1*sin1*8.+n2*sin1*8.+n3*(abs(sin1)*2.+.5)+pow(sin1,2.)*40., 0.);
                vPosition=last;
            ...
            fragmentShader: `
              ...
              varying vec3 vPosition;
              void main(void){
                float opacity=(96.-length(vPosition))/256.*0.6;
                gl_FragColor=vec4(vec3(.6), opacity);
              }
            `,
            transparent: true,
          }),
